...
def main(b, mlp_s, cnn_s, bs, mlp_per, cnn_per, l):
    ...
    if args.data_set == 'ILSVRC2012':
        ...
        model = getattr(torchvision.models, args.model)(pretrained=None)
        
        if hasattr(model, 'fc'):
            model.fc = nn.Linear(model.fc.in_features, 10)
        else:
            raise ValueError(f"Model {args.model} does not have a recognized classifier attribute.")
    ...
    saved_model_name = f'wf{args.weights_file}_ds{args.data_set}_b{bits}_batch{batch_size}_mlpscalar{mlp_scalar}_cnnscalar{cnn_scalar}\
    ...
    with open(LOG_FILE_NAME, 'a') as f:
        ...
        row = [
            args.weights_file, args.model, args.data_set, batch_size, 
